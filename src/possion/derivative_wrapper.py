...
def build_wrapper(cfg, g):
    ...
    x_std = cfg.model.x_std
    train_sample_cnt, eval_sample_cnt = cfg.train.model_sample_cnt, cfg.test.model_sample_cnt
    if cfg.model.derivative == 'steins':
        wrapper = SteinsWrapper(g, x_std=x_std, train_sample_cnt=train_sample_cnt, 
                                         eval_sample_cnt=eval_sample_cnt)
    elif cfg.model.derivative == 'bp':
        wrapper = BackpropWrapper(g, x_std=x_std, train_sample_cnt=train_sample_cnt, 
                                         eval_sample_cnt=eval_sample_cnt)
    elif cfg.model.derivative == 'improved-steins':
        wrapper = ImprovedSteinsWrapper(g, x_std=x_std, train_sample_cnt=train_sample_cnt, 
                                         eval_sample_cnt=eval_sample_cnt)
    else:
...
class GaussianWrapper(Wrapper):
    def __init__(self, g, x_std, train_sample_cnt=8, eval_sample_cnt=16) -> None:
        self.g = g
...
class SteinsWrapper(GaussianWrapper):
    ...
    def dx(self, X, sample_cnt=None):
        ...
        # sample_x.shape: (sample_cnt*batch_size, 2)
        # e_x.shape: (sample_cnt*batch_size, 2)
        sample_X, e_X = gaussian_augment(X, self.x_std, sample_cnt)

        # sample_g.shape: (sample_cnt, batch, 1)
        sample_g = self.g(sample_X).reshape(sample_cnt, batch_size, 1)
        # e_X.shape: (sample_cnt, batch, 2)
        e_X = e_X.reshape(sample_cnt, batch_size, -1)

        # (batch, 2)
        df_dx = torch.mean((sample_g*e_X)/self.x_std**2, dim=0)
    ...
    def dx2(self, X, sample_cnt=None):
        ...
        # (batch_size, 100)
        d2f_dX2 = torch.mean((sample_g*(e_X**2 - self.x_std**2)/self.x_std**4), dim=0)
        return d2f_dX2
...
class ImprovedSteinsWrapper(GaussianWrapper):
    def dx2(self, X, sample_cnt=None):
        ...
        # sample_g_plus/minus.shape: (sample_cnt, batch, 1)
        sample_g_plus = self.g(sample_X_plus).reshape(sample_cnt, batch_size, 1)
        sample_g_minus = self.g(sample_X_minus).reshape(sample_cnt, batch_size, 1)

        e_X = e_X.reshape(sample_cnt, batch_size, -1)
        
        true_g = self.g(X).reshape(1, batch_size, 1)
        d2f_dx2 = (e_X**2-self.x_std**2)*(sample_g_plus+sample_g_minus-2*true_g)/(2*(self.x_std**4))
        # (batch_size, 2)
